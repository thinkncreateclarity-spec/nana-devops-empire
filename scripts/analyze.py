...
def classify(path):
    ...
    return {
        ".py": "Python", ".sh": "Shell", ".md": "Markdown", ".json": "JSON",
        ".toml": "TOML", ".yml": "YAML", ".yaml": "YAML", ".js": "JavaScript",
        ".ts": "TypeScript", ".kt": "Kotlin", ".java": "Java", ".c": "C",
        ".h": "C/C++ Header", ".cpp": "C++", ".rb": "Ruby", ".go": "Go",
        ".rs": "Rust", ".sql": "SQL", ".txt": "Text"
    }.get(ext, "Other")
...
def main():
    ...
    sc = compute_score(ignored_n, untracked_n, len(dirty), ext_counts.get(".md", 0), commits)
    st = health(sc)
    ...
    md_lines.append(f"- Risk score: **{sc}**")
    md_lines.append("")
    ...
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["run_id", "timestamp", "repo_root", "total_files", "tracked_files", "untracked_files", "ignored_files", "top_language", "risk_score", "health_status"])
        if not csv_exists:
            w.writeheader()
        w.writerow({
            "run_id": run_id,
    ...
    snap = {
        "meta": {"run_id": run_id, "timestamp": ts, "repo_root": str(root)},
        "counts": {"total_files": total, "tracked_files": tracked_n, "untracked_files": untracked_n, "ignored_files": ignored_n},
